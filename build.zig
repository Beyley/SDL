...
pub fn createSDL(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode, _sdl_options: SdlOptions) !*std.Build.Step.Compile {
    ...
    switch (target.result.os.tag) {
        ...
        .linux => {
            ...
            //     .style = .{ .cmake = b.path(root_path ++ "include/SDL_config.h.cmake") },
    ...
    if (sdl_options.power_implementation) |chosen| {
        switch (chosen) {
            ...
            .winrt => lib.addCSourceFile(.{ .file = b.path("src/power/winrt/SDL_syspower.cpp"), .flags = c_flags.items }),
            else => |value| {
                const path = try std.mem.concat(b.allocator, u8, &.{ "src/power/", @tagName(value), "/SDL_syspower.c" });
                lib.addCSourceFile(.{ .file = b.path(path), .flags = c_flags.items });
            },
        }
    } else {
    ...
    switch (sdl_options.timer_implementation) {
        .ngage => lib.addCSourceFile(.{ .file = b.path("src/timer/ngage/SDL_systimer.cpp"), .flags = c_flags.items }),
        else => |value| {
            const path = try std.mem.concat(b.allocator, u8, &.{ "src/timer/", @tagName(value), "/SDL_systimer.c" });
            lib.addCSourceFile(.{ .file = b.path(path), .flags = c_flags.items });
        },
    }
    ...
    switch (sdl_options.loadso_implementation) {
            ...
            const path = try std.mem.concat(b.allocator, u8, &.{ "src/loadso/", @tagName(value), "/SDL_sysloadso.c" });
            lib.addCSourceFile(.{ .file = b.path(path), .flags = c_flags.items });
        },
    }
    ...
    switch (sdl_options.locale_implementation) {
        ...
        .haiku => lib.addCSourceFile(.{ .file = b.path("src/locale/haiku/SDL_syslocale.cc"), .flags = c_flags.items }),
        //macos is a .m file (obj-c)
        .macosx => lib.addCSourceFile(.{ .file = b.path("src/locale/macosx/SDL_syslocale.m"), .flags = try std.mem.concat(b.allocator, []const u8, &.{ c_flags.items, &.{"-fobjc-arc"} }) }),
        else => |value| {
            const path = try std.mem.concat(b.allocator, u8, &.{ "src/locale/", @tagName(value), "/SDL_syslocale.c" });
            lib.addCSourceFile(.{ .file = b.path(path), .flags = c_flags.items });
        },
    }
    ...
    { //video implementations
        ...
        if (sdl_options.video_implementations.wayland) {
            ...
            lib.addIncludePath(b.path("include/wayland-protocols"));
    ...
    { //joystick implementations
        if (sdl_options.joystick_implementations.dummy) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/dummy/SDL_sysjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.linux) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/linux/SDL_sysjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.android) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/android/SDL_sysjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.darwin) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/darwin/SDL_iokitjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.bsd) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/bsd/SDL_bsdjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.emscripten) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/emscripten/SDL_sysjoystick.c"), .flags = c_flags.items });
        }
        ...
        if (sdl_options.joystick_implementations.haiku) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/haiku/SDL_haikujoystick.cc"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.n3ds) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/n3ds/SDL_sysjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.os2) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/os2/SDL_os2joystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.ps2) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/ps2/SDL_sysjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.psp) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/psp/SDL_sysjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.virtual) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/virtual/SDL_virtualjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.vita) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/vita/SDL_sysjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.iphoneos) {
            lib.addCSourceFile(.{
                .file = b.path("src/joystick/iphoneos/SDL_mfijoystick.m"),
        ...
        if (sdl_options.joystick_implementations.apple) {
            lib.addCSourceFile(.{
                .file = b.path("src/joystick/apple/SDL_mfijoystick.c"),
                .flags = try std.mem.concat(b.allocator, []const u8, &.{
                    &.{"-fobjc-arc"},
                    c_flags.items,
                }),
        ...
        if (sdl_options.joystick_implementations.dinput or sdl_options.joystick_implementations.xinput or sdl_options.joystick_implementations.rawinput) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/windows/SDL_windowsjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.dinput) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/windows/SDL_dinputjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.xinput) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/windows/SDL_xinputjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.rawinput) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/windows/SDL_rawinputjoystick.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.wgi) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/windows/windows_gaming_input.c"), .flags = c_flags.items });
        }

        if (sdl_options.joystick_implementations.steam) {
            lib.addCSourceFile(.{ .file = b.path("src/joystick/steam/SDL_steamcontroller.c"), .flags = c_flags.items });
        }
    } //joystick implementations

    { //haptic implementations
        if (sdl_options.haptic_implementation == .android) {
            lib.addCSourceFile(.{ .file = b.path("src/haptic/android/SDL_syshaptic.c"), .flags = c_flags.items });
        }

        if (sdl_options.haptic_implementation == .darwin) {
            lib.addCSourceFile(.{ .file = b.path("src/haptic/darwin/SDL_syshaptic.c"), .flags = c_flags.items });
        }

        if (sdl_options.haptic_implementation == .dummy) {
            lib.addCSourceFile(.{ .file = b.path("src/haptic/dummy/SDL_syshaptic.c"), .flags = c_flags.items });
        }

        if (sdl_options.haptic_implementation == .linux) {
            lib.addCSourceFile(.{ .file = b.path("src/haptic/linux/SDL_syshaptic.c"), .flags = c_flags.items });
        }

        if (sdl_options.haptic_implementation == .windows) {
            lib.addCSourceFile(.{ .file = b.path("src/haptic/windows/SDL_dinputhaptic.c"), .flags = c_flags.items });
            lib.addCSourceFile(.{ .file = b.path("src/haptic/windows/SDL_windowshaptic.c"), .flags = c_flags.items });
            lib.addCSourceFile(.{ .file = b.path("src/haptic/windows/SDL_xinputhaptic.c"), .flags = c_flags.items });
        }
    } //haptic implementations

    { //audio implementations
        if (sdl_options.audio_implementations.aaudio) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/aaudio/SDL_aaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.alsa) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/alsa/SDL_alsa_audio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.android) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/android/SDL_androidaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.arts) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/arts/SDL_artsaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.coreaudio) {
            lib.addCSourceFile(.{
                .file = b.path("src/audio/coreaudio/SDL_coreaudio.m"),
                .flags = try std.mem.concat(b.allocator, []const u8, &.{
                    &.{"-fobjc-arc"},
                    c_flags.items,
                }),
        ...
        if (sdl_options.audio_implementations.directsound) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/directsound/SDL_directsound.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.disk) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/disk/SDL_diskaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.dsp) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/dsp/SDL_dspaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.dummy) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/dummy/SDL_dummyaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.emscripten) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/emscripten/SDL_emscriptenaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.esd) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/esd/SDL_esdaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.fusionsound) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/fusionsound/SDL_fsaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.haiku) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/haiku/SDL_haikuaudio.cc"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.jack) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/jack/SDL_jackaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.n3ds) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/n3ds/SDL_n3dsaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.nacl) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/nacl/SDL_naclaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.nas) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/nas/SDL_nasaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.netbsd) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/netbsd/SDL_netbsdaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.openslES) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/openslES/SDL_openslES.c"), .flags = c_flags.items });
            lib.linkSystemLibrary("OpenSLES");
        }

        if (sdl_options.audio_implementations.os2) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/os2/SDL_os2audio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.paudio) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/paudio/SDL_paudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.pipewire) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/pipewire/SDL_pipewire.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.ps2) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/ps2/SDL_ps2audio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.psp) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/psp/SDL_pspaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.pulseaudio) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/pulseaudio/SDL_pulseaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.qsa) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/qsa/SDL_qsa_audio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.sndio) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/sndio/SDL_sndioaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.sun) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/sun/SDL_sunaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.vita) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/vita/SDL_vitaaudio.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.wasapi) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/wasapi/SDL_wasapi.c"), .flags = c_flags.items });
            lib.addCSourceFile(.{ .file = b.path("src/audio/wasapi/SDL_wasapi_win32.c"), .flags = c_flags.items });
            lib.addCSourceFile(.{ .file = b.path("src/audio/wasapi/SDL_wasapi_winrt.c"), .flags = c_flags.items });
        }

        if (sdl_options.audio_implementations.winmm) {
            lib.addCSourceFile(.{ .file = b.path("src/audio/winmm/SDL_winmm.c"), .flags = c_flags.items });
        }
    } //audio implementations
    ...
    { //render implementations
        ...
        if (sdl_options.render_implementations.metal) {
            lib.addCSourceFile(.{
                .file = b.path("src/render/metal/SDL_render_metal.m"),
                .flags = try std.mem.concat(b.allocator, []const u8, &.{
                    &.{"-fobjc-arc"},
                    c_flags.items,
                }),
    ...
    switch (target.result.os.tag) {
        .linux => {
            ...
            if (sdl_options.linux_sdk_path) |linux_sdk_path| {
                ...
                lib.addIncludePath(b.path(b.fmt("{s}/include", .{linux_sdk_path})));
                lib.addLibraryPath(b.path(b.fmt("{s}/lib/{s}", .{ linux_sdk_path, try target.result.linuxTriple(b.allocator) })));
        ...
        .macos => {
            if (sdl_options.osx_sdk_path) |osx_sdk_path| {
                ...
                lib.addFrameworkPath(b.path(b.fmt("{s}/System/Library/Frameworks", .{osx_sdk_path})));
                lib.addSystemIncludePath(b.path(b.fmt("{s}/usr/include", .{osx_sdk_path})));
                lib.addLibraryPath(b.path(b.fmt("{s}/usr/lib", .{osx_sdk_path})));
            } else if (!target.query.isNative()) {
